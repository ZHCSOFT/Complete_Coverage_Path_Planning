...
public:
			...
			std::vector<std::vector<std::vector<cv::Point> > >* paths=NULL)
	{
		...
		{
			std::cout << "[ INFO ] Processing i " << i << " of points.size() " << points.size() << std::endl;
			...
			{
				std::cout << "[ INFO ] Processing j " << j << " of points.size() " << points.size() << std::endl;
				if (j != i)
				{
					if (j > i) //only compute upper right triangle of matrix, rest is symmetrically added
					{
						...
						{
							std::cout << "[ INFO ] direct_connection is true" << std::endl;
							// compute distance
						...
						{
							// A* path planner
							std::cout << "[ INFO ] direct_connection is false, A star path planner enabled" << std::endl;
							if (paths != NULL)
							{
